...
Deno.test("decoder search parses docs and marks an empty result negative", () => {
  const hit = decodeSearchEnvelope(env(
    200,
    JSON.stringify({
      numFound: 2,
  ...
  const empty = decodeSearchEnvelope(env(
    200,
    JSON.stringify({
      numFound: 0,
      docs: [],
...
Deno.test("decoder ignores unknown fields and tolerates absent optional facts", () => {
  const result = decodeRecordEnvelope(env(
    200,
    JSON.stringify({
      key: "/works/OL274505W",
      type: { key: "/type/work" },
...
Deno.test("decoder structured JSON not-found is no_record", () => {
  const missing = decodeRecordEnvelope(env(
    404,
    '{"error":"notfound","key":"/works/OL0000000000W"}',
  ));
  assertEquals(missing.kind, "no_record");
});

Deno.test("decoder detects /type/redirect records and preserves location", () => {
  const redirect = decodeRecordEnvelope(env(
    200,
    JSON.stringify({
      key: "/works/OL45883W",
...
Deno.test("decoder editions page preserves the next link and entries", () => {
  const page = decodeEditionsPageEnvelope(env(
    200,
    JSON.stringify({
